...
def query_nearest_address(lat: float, lon: float, distance: int = 100):
...
def get_building_info_from_point(lat: float, lon: float):
    ...
    if not address:
        return {"status": "not_found", "message": "Nepodařilo se najít adresní místo poblíž zadaných souřadnic."}

    building_code = address.get("stavebniobjekt")
    if not building_code:
        return {"status": "not_found", "message": "Adresní místo bylo nalezeno, ale nemá vazbu na stavební objekt.", "address": address}

    building = query_building_by_code(building_code)
    if not building:
        return {"status": "not_found", "message": "Stavební objekt se nepodařilo načíst.", "address": address, "building_code": building_code}
