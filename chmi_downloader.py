"""
CHMI Metadata CSV Downloader and Archiver
...
def scrape_csv_links() -> List[str]:
    """
    Scrape the index page and extract all CSV download links.
    
    Returns:
        List of absolute URLs for CSV files
    """
    csv_links = []
    ...
    try:
        ...
        print(f"✓ Total CSV files found: {len(csv_links)}\n")
        return csv_links
        ...
        print(f"✗ Unexpected error while parsing index page: {e}")
        return []


def download_csv_file(url: str) -> bool:
    """
    Download a single CSV file to the temp directory and immediately zip it.
    
    Args:
        url: The absolute URL of the CSV file
...
def download_all_csvs(csv_links: List[str]) -> int:
    """
    Download all CSV files from the provided list and individually ZIP each one.
    
    Args:
        csv_links: List of CSV file URLs
    ...
    print(f"\n{'='*60}")
    print(f"DOWNLOADING AND INDIVIDUALLY ZIPPING {len(csv_links)} CSV FILES")
    print(f"{'='*60}\n")
    ...
    for i, url in enumerate(csv_links, 1):
        ...
        if download_csv_file(url):
            successful_downloads += 1
    
    print(f"\n✓ Downloaded and zipped {successful_downloads}/{len(csv_links)} files successfully\n")
...
def main():
    ...
    print("\n" + "="*60)
    print("CHMI METADATA CSV DOWNLOADER AND INDIVIDUAL ZIPPER")
    print("="*60 + "\n")
    
    # Step 1: Ensure temp directory exists
    if not ensure_temp_directory():
        print("✗ Failed to set up temp directory. Exiting.")
        sys.exit(1)
    
    # Step 2: Scrape CSV links from the index page
    csv_links = scrape_csv_links()
    
    if not csv_links:
        print("✗ No CSV files found to download. Exiting.")
        sys.exit(1)
    
    # Step 3: Download all CSV files and individually ZIP them
    downloaded_count = download_all_csvs(csv_links)
    ...
    print("="*60)
    print(f"\nAll files individually zipped in: {TEMP_DIR}\n")
